...
class Database:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            # Ensure assets folder and db file exist
            db_dir = os.path.join(os.path.dirname(__file__), "assets")
            if not os.path.exists(db_dir):
                os.makedirs(db_dir)
            db_path = os.path.join(db_dir, "db_bot.db")
            if not os.path.exists(db_path):
                open(db_path, 'a').close()  # Create empty file if not exists
    ...
    def update_user_warning(self, user_id: int, warns: int) -> None:
        self.cursor.execute(
            f"UPDATE {self.BOT_TABLE} SET WARNINGS=? WHERE ID=?",
            (warns, int(user_id)),
        )
        self.connection.commit()
    ...
    def reset_warnings(self, user_id: int) -> None:
        self.cursor.execute(
            f"UPDATE {self.BOT_TABLE} SET WARNINGS=0 WHERE ID=?",
            (int(user_id),),
        )
        self.connection.commit()
    ...
    def get_info_user(self, user_id: int):
        ...
        return {
            ...
            "WARNINGS": user_data[2],
            "NICK": user_data[3],
            "RANK": user_data[4],
            "STATE": user_data[5],
            "MEMBERSHIP": user_data[6],
            "EXPIRATION": user_data[7],
            "ANTISPAM": user_data[8],
            "CREDITS": user_data[9],
            "REGISTERED": user_data[10],
            "CHECKS": user_data[11],
        }
    ...
    def update_column(self, user_id: int, column: str, value) -> bool | None:
        user_id = int(user_id)

        user_data = self.cursor.execute(
            f"SELECT {column} FROM {self.BOT_TABLE} WHERE ID=?", (user_id,)
        ).fetchone()
